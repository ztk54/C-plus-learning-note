...
```
a在出了函数之外就被**消耗**了，相当于这里的是个野引用，结果不确定，可能会打印随机值
2. ```cpp
	int& r = a;
...
```cpp
const int a = 10;
int& ra = a;          // ❌ 权限放大，编译报错（本身a为const不可更改其大小，但是ra却是int类型的引用，放大了权限）

const int& cra = a;   // ✅ 权限不变

int b = 20;
const int& crb = b;   // ✅ 权限缩小：可以，但不能通过 crb 改 b
```
一句话记：**访问权限只能缩小，不能放大。**
###  临时对象具有常性
...
int i = d;            // OK：隐式类型转换，产生一个"int 临时变量"

int& ri = d;          // ❌ 类型不匹配：临时变量具有常性，普通引用接不住

const int& cri = d;   // ✅ const 引用可以

  

int a = 1;
int& rb = a * 3;      // ❌ a*3 的结果是临时变量，具常性
const int& crb = a * 3;  // ✅

```
**规律**：表达式结果、类型转换结果、函数传值返回值 —— 都是**临时变量**，一律要用 `const` 引用接。
**进阶**：`const` 引用绑定临时对象时，编译器会把临时对象的生命周期**延长到该引用的生命周期**，所以 `const int& cri = d;` 之后用 `cri` 是安全的。
##  指针 vs 引用

| #   | 对比项      | 指针                    | 引用                              |
| --- | -------- | --------------------- | ------------------------------- |
| 1   | 本质       | 保存**地址**的变量           | 变量的**别名**                       |
| 2   | 初始化      | 建议初始化（可以不）            | **必须初始化**                       |
| 3   | 能否改指向    | **可以**改指向别的对象         | **不可以**，一旦绑定终身不变                |
| 4   | 访问方式     | 需要**解引用** `*p`        | **直接用**，编译器自动处理                 |
| 5   | `sizeof` | 地址的字节数（32位 4 / 64位 8） | **引用类型的大小**（`sizeof(int&)` = 4） |
| 6   | 安全性      | 有**空指针、野指针**风险        | 更安全（没有空引用，但要防悬垂）                |
...
p = &b;          // ✅ 指针可以改指向
int& r = a;
r = b;           // ⚠️ 这是"把 b 的值赋给 a"，不是让 r 改指向 b！
```
## inline内联函数
### 为什么要使用inline内联函数
c++内联函数主要是替换c语言的宏函数，宏函数在写法上极易出错`#define Add(a,b)((a)+(b))`这里的括号**一个也不能**省略，不过宏函数能够大大省掉建立函数栈帧的开销，所有我们在写一些简单并且常用的函数时常常会使用宏函数。内联函数写法简单并且还能够起到c语言中宏函数的作用。
```cpp
...
```
### 关键性质
1. **对编译器只是"建议"**：如果函数体过大、含循环/递归，编译器可以拒绝展开。
2. **C++ 设计 inline 的主要目的：替代 C 的宏函数**。
   ```cpp
   // C 的宏：容易写出优先级的坑
   #define ADD(a, b) a + b
   ADD(1, 2) * 3      // 展开成 1 + 2*3 = 7，不是 9！
   #define ADD(a, b) ((a) + (b))   // 必须到处加括号

   // C++ 的 inline：有类型检查，没有宏的那些坑
   inline int Add(int a, int b) 
   { 
	   return a + b; 
   }
   ```
3. **不同编译器行为不一致**，标准没有规定必须展开。
4.  **`inline` 声明与定义分离会导致链接错误**（`LNK2019：无法解析的外部符号`）。
   原因：inline 函数被展开后就没有独立的函数地址，编译器**不会为它生成可链接的符号**，另一个 .cpp 找不到它。
   → **所以 inline 函数的定义必须放在 .h 里**（类内定义的成员函数默认就是 inline，也是这个道理）。

5. **VS 的 Debug 版默认不展开 inline**（方便单步调试），课件给出了两处需要改的设置：
   - `C/C++ → 优化 → 内联函数扩展`（`/Ob1`）
   - `链接器 → 优化 → 启用 COMDAT 折叠`（`/OPT:ICF`）
   * Release 版不受影响。
